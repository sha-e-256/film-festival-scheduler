...
def save_film_objects(film_objects, dst_file_name) -> None:

    with open(file=dst_file_name, mode="wb") as file:
        pickle.dump(obj=film_objects, file=file)
        file.close()

def load_film_objects(src_file_name):

    with open(file=src_file_name, mode="rb") as file:
        film_objects = pickle.load(file=file)
    file.close()
    return film_objects
...
def main() -> None:   
    ...
    if not film_objects_file_exists:
        ...
        for film_data in film_data_list:
            ...
            save_film_objects(film_objects=film_list, dst_file_name=dst_file_name)
            '''
    ...
    film_objects = load_film_objects(file_name)
